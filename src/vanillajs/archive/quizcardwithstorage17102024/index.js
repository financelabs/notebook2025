    ...
    endpoints: (builder) => ({
        ...
        fetchOpenQuizesArray: builder.query({
            async queryFn(array) {
                try {
                    let resArray = [];
                    let res = await timeout(275, window.quizesSets);
...
//         .then((res) => console.log(res));
    ...
    if (!res) {
        ...
        resOpenQuizesArray = await store.dispatch(api.endpoints.fetchOpenQuizesArray.initiate(window.quizesSets.map(item => {
            return { id: item.id }
        })));
