...
async function initialLoad() {
    ...
    if (!res) {
        ...
        let postObject = {
            ...
            email: res?.email,
            user: res?.user,
            avatarUrl: !!res?.avatarUrl ? res.avatarUrl : 'https://images.unsplash.com/photo-1536300099515-6c61b290b654?q=80&w=200&auto=format&fit=crop',
            date: new Intl.DateTimeFormat("ru", {
                weekday: "short",
                year: "numeric",
                month: "short",
        ...
        mediaArray.forEach(item => {
            updates["usersCraft/" + res.userEmail + "/posts/" + item.id] =
            {
                ...postObject,
                content: item.id,
                id: item.id,
                title: !!item?.title ? item.title : "Заголовок",
                comment: !!item?.comment ? item.comment : "Comment",
            }
            updates["usersCraft/" + res.userEmail + "/data/" + item.id] =
                { "html": item.content };
        })
