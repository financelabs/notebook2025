...
import createRtkFirebaseStore, { api, setCurrentDate, setOpenAvatars } from "../../../utlities/createRtkFirebaseStore";
...
const store = createRtkFirebaseStore(); // createRtkFirebaseDbAuthStore();
console.log(store);
...
function doShowQuizFromQuizSet(id) {
    console.log(id);
  let quiz =  window.quizesSets.find(quiz => quiz.id === id);
    const myModal = new bootstrap.Modal('#quizModal', {
        keyboard: false
      });
      $("#quiztitle").innerHTML = quiz.title;
      $("#quizbody").innerHTML = quiz.text;
      myModal.show();
}
...
async function doShowUserQuizPosts(userEmail) {
    ...
    $("#quizbuttonslist").innerHTML = window.quizesSets.map((item, index) => {
        ...
        class='${foundQuiz(
            ...
            userUniquePosts
                ...
                ? "btn btn-sm btn-success page m-1 selectquiz" : "btn btn-sm btn-outline-secondary page m-1 selectquiz"}'
    ...
    let userPostsShowButtons = [...$$('.selectquiz')];
    userPostsShowButtons.forEach(function (btn) {
        btn.addEventListener('click', function (event) {
            doShowQuizFromQuizSet(event.currentTarget.getAttribute("id"))
        }, false);
    })
